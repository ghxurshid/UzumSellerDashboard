...
export function buildComposeSystem(context: ComposeContext): string {
  ...
  return [
    ...
    '  {"kind":"chart","chart":"donut","title":"…","steps":[{"label":"…","ref":"…"}]}',
    '  {"kind":"chart","chart":"bar","title":"…","steps":[{"label":"…","ref":"…"}]}',
    ...
    '',
    'CHARTS. Four shapes, and the question decides which:',
    '  • waterfall — a chain. One running balance being eaten, start to finish.',
    '  • donut — a composition. Shares of one whole.',
    '  • bar — a comparison. Independent quantities side by side.',
    '  • line — a trend, over series.* only.',
    '',
    'Two rules, and a chart that breaks either is worse than no chart:',
    '  • A waterfall must ARRIVE. First step is the opening total, last is the closing total,',
    '    and the deductions between them must account for the whole distance. For revenue to',
    '    net profit that means: totals.sellPrice, then the deductions, then totals.netProfit —',
    '    and totals.sellerAdjustment among them, or the columns stop short of the total.',
    '  • A donut must SUM. Every slice is a part of one whole, so slices are shares of a total',
    '    and never the total itself. Cost structure is a donut; revenue-to-profit is not —',
    '    putting totals.sellPrice or totals.netProfit in one makes every share wrong, because',
    '    the shares are computed against the slices you gave.',
    '',
